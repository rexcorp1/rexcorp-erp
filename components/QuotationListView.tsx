...
const QuotationListView: React.FC<QuotationListViewProps> = ({ 
    ...
    const getStatusBadge = (status: Quotation['status']) => {
        ...
        switch (status) {
            case 'Open': return <span className={`bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300 ${baseClasses}`}>{status}</span>;
            case 'Cancelled': return <span className={`bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300 ${baseClasses}`}>{status}</span>;
            case 'Expired': return <span className={`bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 ${baseClasses}`}>{status}</span>;
                ...
                <div className="flex-1 rounded-lg border border-gray-200 bg-white dark:bg-gray-800 dark:border-gray-700 flex flex-col">
                    ...
                    <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 p-4">
                        <div className="flex items-center space-x-2">
                            ...
                            <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Quotation</h1>
                        </div>
                        <div className="flex items-center space-x-2">
                        ...
                        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead className="bg-gray-50 text-xs uppercase text-gray-700 sticky top-0 dark:bg-gray-700/50 dark:text-gray-300">
                                <tr>
                                    ...
                                    <th scope="col" className="px-6 py-3 font-semibold">Grand Total</th>
                            ...
                            <tbody>
                                ...
                                {filteredQuotations.length === 0 && (
                                    <tr>
                                        <td colSpan={7} className="py-8 text-center text-gray-500 dark:text-gray-400 italic">No quotations found.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
