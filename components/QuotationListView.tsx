...
interface QuotationListViewProps {
    ...
    onQuotationSelect: (quotation: Quotation) => void;
    onAddQuotation: () => void;
}
            ...
            <div className="flex-1 min-h-0 rounded-xl border border-gray-200 bg-white dark:bg-gray-900 dark:border-gray-800 flex flex-col overflow-hidden shadow-sm">
                ...
                <div className="flex flex-shrink-0 items-center justify-between border-b border-gray-200 px-6 py-3 dark:border-gray-800">
                    <div className="flex items-center space-x-3">
                        <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Quotation</h1>
                    </div>
                    <div className="flex items-center space-x-2">
                        ...
                        <Button onClick={onAddQuotation} className="h-9 px-4 font-semibold bg-gray-900 text-white hover:bg-gray-800 dark:bg-blue-600 dark:hover:bg-blue-700">
                            + Add Quotation
                        </Button>
                    </div>
                </div>
                ...
                <div className="flex flex-shrink-0 flex-wrap items-center justify-between gap-4 border-b border-gray-200 px-6 py-2.5 dark:border-gray-800 bg-white dark:bg-gray-900 z-20">
                    ...
                    <div className="flex items-center space-x-4 text-sm font-medium text-gray-500 dark:text-gray-400">
                        ...
                        <span>{filteredQuotations.length} of {quotations.length}</span>
                    ...
                    <table className="w-full text-sm text-left text-gray-600 dark:text-gray-400 border-collapse">
                        <thead className="text-xs uppercase text-gray-500 sticky top-0 z-10 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-800 shadow-[inset_0_-1px_0_#e5e7eb] dark:shadow-[inset_0_-1px_0_#1f2937]">
                            <tr>
                                ...
                                <th scope="col" className="px-4 py-2.5 font-semibold tracking-wider">Client Name</th>
                                ...
                                <th scope="col" className="px-4 py-2.5 font-semibold tracking-wider">Grand Total</th>
                        ...
                        <tbody>
                            {filteredQuotations.map(quote => (
                                <tr key={quote.id} className="bg-white border-b border-gray-100 hover:bg-gray-50/80 transition-colors dark:bg-gray-900 dark:border-gray-800 dark:hover:bg-gray-800/50 group">
                                    <td className="w-4 p-3"><Checkbox className="rounded border-gray-300"/></td>
                                    <td className="px-4 py-1.5 whitespace-nowrap">
                                        <button onClick={() => onQuotationSelect(quote)} className="font-semibold text-gray-900 dark:text-white hover:underline transition-all">
                                            {quote.clientName}
                                        </button>
                                    </td>
                                    ...
                                    <td className="px-4 py-1.5 font-medium text-gray-900 dark:text-gray-300">{quote.date}</td>
                                    <td className="px-4 py-1.5 text-gray-700 dark:text-gray-300">{quote.grandTotal}</td>
                                    <td className="px-4 py-1.5 text-xs text-gray-500">{quote.quotationId}</td>
                                    <td className="px-4 py-1.5">
                                        <div className="flex items-center justify-end space-x-4 text-gray-400">
                                            <span className="text-[10px] whitespace-nowrap">2 h</span>
                                            <div className="flex items-center space-x-1 hover:text-gray-700 cursor-pointer transition-colors">
                                                <CommentIcon className="w-4 h-4"/>
                                                <span className="text-xs">{quote.comments}</span>
                                            </div>
                                ...
                                <tr>
                                    <td colSpan={7} className="py-16 text-center text-gray-500 bg-gray-50/30 dark:bg-gray-800/20">
                                        <div className="flex flex-col items-center justify-center">
                                            <p className="text-base font-medium text-gray-900 dark:text-gray-100">No quotations found</p>
                ...
                <div className="flex-shrink-0 flex items-center justify-between px-6 py-3 bg-white border-t border-gray-200 dark:bg-gray-900 dark:border-gray-800 z-20">
                    <div className="text-sm text-gray-500">
                        Showing <span className="font-medium text-gray-900 dark:text-white">{filteredQuotations.length}</span> of <span className="font-medium text-gray-900 dark:text-white">{quotations.length}</span> entries
                    </div>
